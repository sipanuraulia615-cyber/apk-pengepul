...
```
    ...
    <main class="flex-1 p-10 bg-white">
        <header class="mb-8">
            <h1 class="text-xl font-medium text-gray-900">Laporan Transaksi Pembelian</h1>
            <p class="text-xs text-gray-400 mt-0.5">Data pembelian sayuran dari petani</p>
        </header>
            ...
            <table class="w-full text-left text-sm border-collapse">
                <thead>
                    <tr class="text-xs text-gray-400 border-b border-gray-100">
                        ...
                        <th class="pb-4 font-medium w-[15%]">Petani</th>
                        <th class="pb-4 font-medium w-[15%]">Sayuran</th>
                        <th class="pb-4 font-medium w-[14%]">Harga/kg</th>
                        <th class="pb-4 font-medium w-[12%]">Jumlah</th>
                        <th class="pb-4 font-medium w-[14%]">Total</th>
                        <th class="pb-4 font-medium w-[10%]">Waktu</th>
                    </tr>
                </thead>

                <tbody class="text-gray-700 divide-y divide-gray-50">
                    @forelse($laporan_beli as $index => $data)
                    <tr class="align-middle">
                        ...
                        <td class="py-4">
                            <span class="px-2.5 py-1 text-xs bg-blue-50 text-blue-600 rounded-full font-medium">
                                BEL{{ str_pad($data->id_beli, 3, '0', STR_PAD_LEFT) }}
                            </span>
                        </td>
                        ...
                        <td class="py-4">
                            Rp {{ number_format($data->harga_per_kg, 0, ',', '.') }}
                        </td>
                        ...
                        <td class="py-4 text-xs text-gray-500">
                            {{ date('d/m H:i', strtotime($data->waktu_beli)) }}
                        </td>
                    </tr>
                    @empty
                    <tr>
        ...
        <div class="max-w-5xl flex justify-between items-center text-xs text-gray-500">
            <p>Menampilkan {{ count($laporan_beli) }} data transaksi pembelian</p>

            <div class="flex items-center space-x-2">
                <button class="w-7 h-7 bg-[#0d47a1] text-white flex items-center justify-center rounded font-medium shadow-sm">
                    1
                </button>
            </div>
        </div>
