<x-app-layout>
    <div class="flex min-h-screen bg-gray-50 text-gray-800 -mt-6">
            ...
            <div>
                ...
                <div class="mb-4">
                    ...
                    <nav class="space-y-1">
                        ...
                        <a href="{{ route('transaksijual') }}" class="flex items-center px-4 py-2.5 bg-blue-900 bg-opacity-40 text-white rounded-md text-sm font-medium">Transaksi Jual</a>
        ...
        <main class="flex-1 p-10 bg-white">
            <header class="flex justify-between items-start mb-8">
                <div>
                    <h1 class="text-xl font-medium text-gray-900">Transaksi Jual</h1>
                    <p class="text-xs text-gray-500 mt-0.5">Input penjualan sayuran ke mitra / pasar</p>
                </div>
            </header>
            ...
            <section class="max-w-4xl bg-white rounded-lg mb-12">
                <h2 id="formTitle" class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-6">Form Transaksi Penjualan</h2>

                <form id="formJual" method="POST" action="{{ route('transaksijual.store') }}">
                    @csrf
                    ...
                    <div class="grid grid-cols-2 gap-x-12 gap-y-6 text-sm mb-8">
                        <div>
                            ...
                            <select name="id_sayur" id="id_sayur" onchange="updateIdSayur(this)" required class="w-full border-gray-300 rounded-lg p-2 focus:border-blue-500 focus:ring-0 text-sm">
                                ...
                                @foreach($data_sayur ?? [] as $sayur)
                                    <option value="{{ $sayur->id_sayur }}" data-id="{{ $sayur->id_sayur }}" data-stok="{{ $sayur->jumlah_stok ?? 0 }}">
                                        {{ $sayur->nama_sayur }} (Stok: {{ $sayur->jumlah_stok ?? 0 }} kg)
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="harga_per_kg" id="harga_per_kg" oninput="hitungTotal()" placeholder="Contoh: 7000" required class="w-full border-gray-300 rounded-lg p-2 focus:border-blue-500 focus:ring-0 text-sm">
                        </div>
                        <div>
                            <label class="block text-gray-700 font-medium mb-1.5">Jumlah (kg)</label>
                            <input type="number" name="jumlah_kg" id="jumlah_kg" oninput="hitungTotal()" placeholder="Contoh: 150" required class="w-full border-gray-300 rounded-lg p-2 focus:border-blue-500 focus:ring-0 text-sm">
                        </div>
                        ...
                        <div>
                            <label class="block text-gray-500 font-medium mb-1.5">Waktu Penjualan</label>
            ...
            <section class="max-w-4xl">
                <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">Riwayat Transaksi Jual</h3>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm text-gray-700">
                            @forelse($transaksi_jual ?? [] as $tj)
                            <tr class="border-b border-gray-100 hover:bg-gray-50/50 transition">
                                <td class="py-4"><span class="px-2 py-0.5 text-xs bg-green-50 text-green-600 rounded font-mono font-medium">JAL{{ sprintf('%03d', $tj->id_jual) }}</span></td>
                                <td class="py-4 font-medium">{{ $tj->nama_sayur }}</td>
                                <td class="py-4">Rp {{ number_format($tj->harga_per_kg, 0, ',', '.') }}</td>
                                <td class="py-4">{{ $tj->jumlah_kg }} kg</td>
                                <td class="py-4 font-semibold">Rp {{ number_format($tj->total_bayar, 0, ',', '.') }}</td>
                                <td class="py-4 text-xs text-gray-500 font-mono">{{ date('d/m H:i', strtotime($tj->waktu_jual)) }}</td>
                                <td class="py-4 flex items-center justify-center space-x-3">
                                    <button type="button" onclick="pemicuEdit('{{ $tj->id_jual }}', '{{ $tj->id_sayur }}', '{{ $tj->harga_per_kg }}', '{{ $tj->jumlah_kg }}')" class="text-xs text-blue-600 hover:underline font-medium">Ubah</button>
                                    <form method="POST" action="{{ route('transaksijual.destroy', $tj->id_jual) }}" onsubmit="return confirm('Hapus data transaksi ini?')">
                                        @csrf
                                        @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="text-center py-8 text-gray-400 text-xs italic">Belum ada transaksi penjualan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        ...
        function updateIdSayur(select) {
            ...
            document.getElementById('display_id_sayur').value = id ? 'S' + String(id).padStart(3, '0') : '-';
        }
        ...
        function hitungTotal() {
            ...
            document.getElementById('display_total_bayar').innerText = 'Rp ' + (harga * jumlah).toLocaleString('id-ID');
        }

        function pemicuEdit(idJual, idSayur, harga, jumlah) {
            document.getElementById('formTitle').innerText = 'Edit Form Transaksi Penjualan';
        ...
        function resetToTambah() {
            ...
            document.getElementById('formJual').action = "{{ route('transaksijual.store') }}";
